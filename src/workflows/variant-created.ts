  ...
  async ({ product_variants, additional_data }, { container }) => {
    ...
    try {
      ...
      try {
        product_variants.forEach((variant, index) => {
          try {
            ...
            if (!variant) {
              logger.warn(
                `Variant at index ${index} is null or undefined, skipping`,
              );
              return;
            }
            ...
            if (!variant.title) {
              logger.warn(
                `Variant at index ${index} missing title, using fallback`,
              );
            }

            // Debug logging for variant structure
            logger.info(
              `Processing variant ${index}: ${JSON.stringify(
                {
                  id: variant.id,
                  title: variant.title,
            ...
            const params: CreateProductParams = {
              ...
              money_amounts: Array.isArray(variant.prices)
                ? variant.prices
                : undefined,
            };
            ...
            logger.error(
              `Error processing variant at index ${index}:`,
              variantError,
            );
      ...
      if (payloads.length === 0) {
        logger.warn(
          "No valid payloads created, skipping Stripe product creation",
        );
        return;
      }
      ...
      const results = await Promise.allSettled(
        payloads.map(async (payload, index) => {
          try {
            logger.info(
              `Creating Stripe product for payload ${index + 1}/${payloads.length}: ${payload.name}`,
            );

            let product;
            logger.info("Creating basic product without specific pricing");
            product =
              await stripeProductModuleService.createStripeProduct(payload);

            if (product?.id) {
              logger.info(
                `Successfully created Stripe product: ${product.id} for variant: ${payload.name}`,
              );
              return {
                success: true,
            ...
            logger.error(
              `Failed to create Stripe product for variant "${payload.name}":`,
              productError,
            );
            return {
              success: false,
      ...
      logger.info(
        `Stripe product creation summary: ${successful.length} successful, ${failed.length} failed out of ${results.length} total`,
      );
      ...
      successful.forEach((result) => {
        if (result.status === "fulfilled" && result.value.success) {
          logger.info(
            `✅ Created: ${result.value.productId} for variant: ${result.value.variantName}`,
          );
        }
      });
      ...
      failed.forEach((result) => {
        if (result.status === "rejected") {
          ...
          logger.error(
            `❌ Failed: ${result.value.variantName} - ${result.value.error}`,
          );
        }
      });
      ...
      if (failureRate > 0.5) {
        // More than 50% failed
        logger.error(
          `High failure rate detected: ${Math.round(failureRate * 100)}% of Stripe product creations failed`,
        );
      ...
      logger.error(
        "Critical error in productVariantCreated hook:",
        globalError,
      );
