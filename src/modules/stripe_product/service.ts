...
class StripeProductModuleService extends MedusaService({
  ...
  constructor(
    { logger, stripeProductRepository }: InjectedDependencies,
  ...
  private async sendRequest(url: string, method: string, data?: any) {
    ...
    this.logger_.info(
      `API Key: ${JSON.stringify(this.options_.api_secret, null, 2)}`,
    );
  }
  ...
  private mapMoneyAmountsToStripePrices(
    ...
    try {
      ...
      return moneyAmounts
        .map((moneyAmount, index) => {
          try {
            if (!moneyAmount) {
              this.logger_.warn(
                `Money amount at index ${index} is null/undefined, skipping`,
              );
              return null;
            }
            ...
            this.logger_.error(
              `Error processing money amount at index ${index}:`,
              itemError,
            );
            return null;
          }
  ...
  async createStripeProduct(
    ...
    try {
      ...
      this.logger_.info(
        `Successfully created Stripe product with ID: ${stripeProduct.id}`,
      );
  ...
  async createProductWithPricesFromMoneyAmounts(
    ...
    try {
      this.logger_.info(
        `Creating Stripe product with money amounts: ${productData.name}`,
      );
      ...
      const priceParams = this.mapMoneyAmountsToStripePrices(
        productData.money_amounts,
        isRecurring,
      );
      ...
      this.logger_.info(
        `Successfully created Stripe product ${product.id} with ${prices.length} price(s)`,
      );
      ...
      this.logger_.error(
        `Failed to create Stripe product with money amounts: ${error.message}`,
      );
      throw new Error(
        `Failed to create Stripe product with money amounts: ${error.message}`,
      );
  ...
  async updateStripeProduct(
    ...
    try {
      ...
      this.logger_.error(
        `Failed to update Stripe product ${productId}: ${error.message}`,
      );
